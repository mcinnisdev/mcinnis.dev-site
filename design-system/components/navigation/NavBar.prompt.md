Sticky site header — mark + wordmark + `mcinnis.dev` sub-line on the left, uppercase mono links on the right in 18.4°-skewed slabs (the logo's stroke angle). The active item's slab fills orange with a 3px bar along the header's base; hover tints the slab.
